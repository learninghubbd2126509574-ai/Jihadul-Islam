          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-slate-200 animate-slide-up">
            ...
            {supportSubmitted ? (
              <div className="text-center py-6 space-y-3 animate-scale-up">
                ...
                <h4 className="font-bold text-slate-800 text-sm">
                  {lang === 'bn' ? 'বার্তাটি সফলভাবে জমা হয়েছে!' : 'Message Submitted Successfully!'}
                </h4>
                <p className="text-slate-400 text-xs">
                  {lang === 'bn'
                    ? 'আমাদের একজন সাপোর্ট এজেন্ট ১ মিনিটের মধ্যে আপনার সাথে যোগাযোগ করবে।'
                    : 'A support representative will reach back in a moment in your simulated dashboard.'}
                </p>
              </div>
            ) : (
              <form onSubmit={handleSupportSubmit} className="space-y-4">
                <div className="bg-blue-50 border border-blue-100 p-3 rounded-xl text-[11px] text-blue-800 font-medium leading-relaxed">
                  {lang === 'bn'
                    ? 'আপনার কাজ বা পেমেন্ট সংক্রান্ত যেকোনো সমস্যায় আমাদের সাপোর্ট টিমের সাহায্য নিন।'
                    : 'Having queries with typing speeds or lead sheets? Write to our live training representative.'}
                </div>

                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 block">{lang === 'bn' ? 'সমস্যার বিষয় (Subject):' : 'Topic / Issue:'}</label>
                  <select className="w-full border border-slate-200 bg-white rounded-xl p-2.5 text-xs text-slate-700 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                    <option>{lang === 'bn' ? 'টাইপিং কাজ সংক্রান্ত' : 'Typing Job Query'}</option>
                    <option>{lang === 'bn' ? 'মার্কেটিং কাজ সংক্রান্ত' : 'Marketing Campaign Issue'}</option>
                    <option>{lang === 'bn' ? 'ডেমো ব্যালেন্স বা শপ আইটেম' : 'Booster / Balance Issue'}</option>
                    <option>{lang === 'bn' ? 'অন্যান্য জিজ্ঞাসা' : 'Other Query'}</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-600 block">{lang === 'bn' ? 'আপনার মেসেজ লিখুন:' : 'Write Message:'}</label>
                  <textarea
                    required
                    value={supportMessage}
                    onChange={(e) => setSupportMessage(e.target.value)}
                    placeholder={lang === 'bn' ? 'বিস্তারিত এখানে লিখুন...' : 'Write your issue details here...'}
                    className="w-full h-24 border border-slate-200 rounded-xl p-3 text-xs focus:border-blue-500 outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>

                <button
                  type="submit"
                  className="w-full bg-[#0f172a] hover:bg-slate-800 text-white font-bold py-3 rounded-xl text-xs transition-all active:scale-95 shadow-sm"
                >
                  {lang === 'bn' ? 'বার্তা পাঠান' : 'Send Support Request'}
                </button>
              </form>
            )}
          </div>
